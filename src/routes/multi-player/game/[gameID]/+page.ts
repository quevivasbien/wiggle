...
export async function load(event: LoadEvent) {
    ...
    // get game data
    const gameData = await (await event.fetch(`${base}/api/active-game?gameID=${gameID}`)).json();
    ...
    const response = await event.fetch(`${base}/api/active-game?gameID=${gameID}&subscribe=true`);
    ...
    const submitWord = async (word: string) => {
        ...
        const url = `${base}/api/active-game?gameID=${gameID}&playerID=${myIDValue}&word=${word}`;
    ...
    const quit = () => {
        ...
        event.fetch(`${base}/api/active-game?gameID=${gameID}&playerID=${myIDValue}`, { method: "DELETE" })
    }
